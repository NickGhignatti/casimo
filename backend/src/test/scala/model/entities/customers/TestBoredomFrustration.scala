...
class TestBoredomFrustration extends AnyFunSuite:
  ...
  test("creating a class with a legal Boredom or Frustration format should store the amount"):
    val mock = MockBF(boredom = 70.0, frustration = 20.0)
    assert(mock.boredom === 70.0)
    assert(mock.frustration === 20.0)

  test("creating a class with a non percentile value of boredom or frustration should throw IllegalArgumentException"):
    ...
    assert(exB.getMessage === s"requirement failed: Boredom must be a percentile value, instead is $negativeValue %")
    assert(exF.getMessage === s"requirement failed: Frustration must be a percentile value, instead is $negativeValue %")

  test("Modifying the boredom or frustration to a new valid amount should update the amount"):
  ...
  test("Modifying the Boredom or frustration to an invalid amount should clip to the max or min value"):
    val startValue = 90.0
