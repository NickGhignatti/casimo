...
class TestDecisionTree extends AnyFunSuite with Matchers:
  ...
  test("DecisionNode should choose the branches correctly"):
    val node = DecisionNode[Customer, String](
      _.boredom >= 50,
      Leaf(_ => "high"),
      Leaf(_ => "low")
    )
  ...
  test("MultiNode selects branch or throws"):
    val m = MultiNode[Customer, RiskProfile, String](
      keyOf = _.riskProfile,
      branches = Map(
        VIP -> Leaf(_ => "VIP profile"),
        Regular -> Leaf(_ => "Regular Profile")
      ),
  ...
  test("Nested DecisionTree evaluation"):
    val tree: DecisionTree[Customer, String] =
      DecisionNode[Customer, String](
        ...
        trueBranch = MultiNode(
          ...
          branches = Map(
            Blackjack -> Leaf(_ => "customer is a VIP playing Blackjack"),
            Roulette -> Leaf(_ => "customer is a VIP playing Roulette")
          ),
          default =
            Leaf(_ => "customer is a VIP playing playing an unknown game")
        ),
    ...
    tree.eval(
      Customer()
        .withProfile(VIP)
        .withCustomerState(Playing(GameBuilder.blackjack(Vector2D.zero)))
    ) shouldBe "customer is a VIP playing Blackjack"
    tree.eval(
      Customer()
        .withProfile(VIP)
        .withCustomerState(Playing(GameBuilder.roulette(Vector2D.zero)))
    ) shouldBe "customer is a VIP playing Roulette"
    tree.eval(
      Customer()
        .withProfile(VIP)
        .withCustomerState(Playing(GameBuilder.slot(Vector2D.zero)))
    ) shouldBe "customer is a VIP playing playing an unknown game"
